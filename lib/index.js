...
Sweet.prototype = {
	...
	applyOptions: function(o) {
		var upper = function(m) { return m[1].toUpperCase(); };
		for (var key in o) {
			this[key.replace(/_([a-z])/g, upper)] = o[key];
		}
	},

	prepareContent: function() {
		var contentFiles = getFilesList(this.contentDir);
		this.numberOfFiles = contentFiles.length;
		for (var contentIdx = 0, contentCnt = this.numberOfFiles; contentIdx < contentCnt; contentIdx++) {
	...
	getFileSignificantPath: function(filepath) {
		...
		filepath = filepath.replace(path.join(this.contentDir, this.getFileLanguage(filepath)), '');
	...
	getContent: function(file) {
		...
		if (parts.length !== 2) throw new Error('Head/body delimiter (---) not found.');
	...
	fileToLink: function(filepath, prefixes) {
		...
		if (prefixes) {
			if (this.langs) {
				return prefixes[this.getFileLanguage(filepath)] + link;
			}
			else {
