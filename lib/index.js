...
Sweet.prototype = {
	...
	prepareContent: function() {
		...
		for (var contentIdx = 0, contentCnt = this.filesToProcess; contentIdx < contentCnt; contentIdx++) {
			var srcPath = contentFiles[contentIdx],
				...
				fileId = (lang && lang !== this.rootLang ? lang + '/' : '') + this.getFileSignificantPath(srcPath);
	...
	compileTemplates: function() {
		for (var templateId in this.templates) {
			...
			var compiled;
			try {
				compiled = fest.compile(toUnixPath(templatePath), {beautify: !this.debug});
			}
			catch (e) {
				// @todo Show project relative path
				// @todo file:line format
				this.error('Cannot compile template ' + templatePath + '\n' + e.message);
			}
